...
primGrid <- function(input, y, probs, threshold = 0, vdim = 2, ...) {
  ...
  for (i in seq_len(num_com)) {
    ...
    points_box_index <- lapply(1:num, function(s)
      suppressMessages(semi_join(points_all_x, points_box_x[[s]])) %$% Index)
    points_box_probs <- lapply(points_box_index, function(s) probs[s])

    # Points of interest in the box & domain
    interest_box <- sapply(1:num, function(s)
      points_box_y[[s]][points_box_y[[s]] > threshold])
    ...
    # Scenario mass
    df$mass <- sapply(1:num, function(s)
      round(length(points_box_y[[s]])/length(points_all_y), 3))

    # Scenario weighted density
    df$wdens <- sapply(1:num, function(s)
      round(sum(interest_box[[s]])/sum(points_box_probs[[s]]), 3))

    # scenario weighted coverage
    df$wcov <- sapply(1:num, function(s)
      round(sum(interest_box[[s]])/sum(interest_all), 3))
...
primSelect <- function(input, y, probs, var, box, ...) {
  ...
  points_box_index <- suppressMessages(
    semi_join(points_all_x, points_box_x) %$% Index)
