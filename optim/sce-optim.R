...
## From hydromad package

################################################################################

sceDefaults <- function()
    list(
      ncomplex = 10, ## number of complexes
      cce.iter = NA, ## number of iteration in inner loop (CCE algorithm)
      fnscale = 1,   ## function scaling factor (set to -1 for maximisation)
      elitism = 1,   ## controls amount of weighting in sampling
      initsample = "latin", ## sampling scheme -- "latin" or "random"
      reltol = 1e-5, ## convergence threshold
      tolsteps = 7,  ## number of iterations within reltol for convergence
      maxit = 1000,  ## (1000) maximum number of iterations
      maxeval = Inf, ## maximum number of function evaluations
      maxtime = Inf, ## maximum duration of optimization in seconds
      returnpop = F, ## whether to return populations from all iterations
